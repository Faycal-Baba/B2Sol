...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(B_weth),deposit)==(msg_sender: USERS & msg_value: NAT1 | balanceOf(msg_sender)-msg_value: NAT & accountOf(msg_sender)+msg_value: NAT & balanceOf(THIS)+msg_value: NAT ==> (msg_sender: ADDRESS & THIS: ADDRESS & THIS/=msg_sender & msg_value: NAT & balanceOf(THIS)+msg_value: NAT & balanceOf(msg_sender)-msg_value: NAT & {msg_sender|->accountOf(msg_sender)+msg_value}: ADDRESS +-> NAT | balanceOf:=balanceOf<+{msg_sender|->balanceOf(msg_sender)-msg_value,THIS|->balanceOf(THIS)+msg_value} || accountOf:=accountOf<+{msg_sender|->accountOf(msg_sender)+msg_value} || (accountOf(msg_sender)+msg_value>=1 & msg_sender/:depositors & card(depositors)<threshold ==> depositors:=depositors\/{msg_sender} [] not(accountOf(msg_sender)+msg_value>=1 & msg_sender/:depositors & card(depositors)<threshold) ==> skip)) [] not(balanceOf(msg_sender)-msg_value: NAT & accountOf(msg_sender)+msg_value: NAT & balanceOf(THIS)+msg_value: NAT) ==> skip);
  List_Substitution(Machine(B_weth),deposit)==(IF balanceOf(msg_sender)-msg_value: NAT & accountOf(msg_sender)+msg_value: NAT & balanceOf(THIS)+msg_value: NAT THEN transfer(msg_sender,THIS,msg_value) || set_accountOf_abstract({msg_sender|->accountOf(msg_sender)+msg_value}) || IF accountOf(msg_sender)+msg_value>=1 & msg_sender/:depositors & card(depositors)<threshold THEN depositors:=depositors\/{msg_sender} END END);
  ...
  List_Substitution(Machine(B_weth),transferTo)==(IF accountOf(msg_sender)>amount & accountOf(msg_sender)-amount: NAT & msg_sender/=dst & accountOf(dst)+amount: NAT THEN set_accountOf_abstract({msg_sender|->accountOf(msg_sender)-amount,dst|->accountOf(dst)+amount}) END);
