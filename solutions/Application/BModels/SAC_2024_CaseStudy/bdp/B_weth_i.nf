...
THEORY ListInvariantX IS
  ...
  List_Invariant(Implementation(B_weth_i))==(index: NAT & donated_i: BOOL & depositors_i: 0..99 --> depositors & manager_i: USERS & manager_i = manager & donated_i = donated & index = card(depositors) & ran(0..index-1<|depositors_i) = depositors & 0..index-1<|depositors_i: 0..index-1 >-> ADDRESS & dom(depositedOver_100|>{TRUE}) = depositors)
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Implementation(B_weth_i))==(check_account_overflow,deposit,withdraw,transferTo,approve,transferFrom,rewardTopDepositors);
  List_Operations(Implementation(B_weth_i))==(check_account_overflow,deposit,withdraw,transferTo,approve,transferFrom,rewardTopDepositors)
END
&
THEORY ListInputX IS
  List_Input(Implementation(B_weth_i),check_account_overflow)==(?);
...
THEORY ListOutputX IS
  List_Output(Implementation(B_weth_i),check_account_overflow)==(ret);
...
THEORY ListHeaderX IS
  List_Header(Implementation(B_weth_i),check_account_overflow)==(ret <-- check_account_overflow);
...
THEORY ListPreconditionX IS
  Own_Precondition(Implementation(B_weth_i),check_account_overflow)==(btrue);
  List_Precondition(Implementation(B_weth_i),check_account_overflow)==(btrue);
...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Implementation(B_weth_i),rewardTopDepositors)==(msg_sender: USERS & msg_value: NAT | @(thisBalance,managerBalance,safe).((btrue | safe:=bool(!xx.(xx: ran(0..index-1<|depositors_i) => accountOf(xx)+1: NAT)));(THIS: ADDRESS | thisBalance:=balanceOf(THIS));(manager_i: ADDRESS | managerBalance:=balanceOf(manager_i));(msg_value = threshold_i & index = threshold_i & donated_i = FALSE & thisBalance+msg_value<=MAXINT & managerBalance-msg_value>=0 & safe = TRUE ==> ((msg_sender: ADDRESS & THIS: ADDRESS & THIS/=msg_sender & msg_value: NAT & balanceOf(THIS)+msg_value: NAT & balanceOf(msg_sender)-msg_value: NAT | balanceOf:=balanceOf<+{msg_sender|->balanceOf(msg_sender)-msg_value,THIS|->balanceOf(THIS)+msg_value});donated_i:=TRUE;@ii.(ii:=0;WHILE ii<index DO @depositorBalance.((depositors_i(ii): ADDRESS | depositorBalance:=accountOf(depositors_i(ii)));(depositors_i(ii): ADDRESS & depositorBalance+1: NAT | accountOf:=accountOf<+{depositors_i(ii)|->depositorBalance+1});ii:=ii+1) INVARIANT ii = 100 or ii: dom(depositors_i) & accountOf = accountOf$0<+%xx.(xx: depositors_i[0..ii-1] | accountOf$0(xx)+1) VARIANT 100-ii END)) [] not(msg_value = threshold_i & index = threshold_i & donated_i = FALSE & thisBalance+msg_value<=MAXINT & managerBalance-msg_value>=0 & safe = TRUE) ==> skip)));
  ...
  Expanded_List_Substitution(Implementation(B_weth_i),transferTo)==(msg_sender: USERS & dst: USERS & amount: NAT1 | @(senderBalance,receiverBalance).((msg_sender: ADDRESS | senderBalance:=accountOf(msg_sender));(dst: ADDRESS | receiverBalance:=accountOf(dst));(senderBalance>=amount & receiverBalance+amount<=MAXINT ==> ((msg_sender: ADDRESS & senderBalance-amount: NAT | accountOf:=accountOf<+{msg_sender|->senderBalance-amount});(dst: ADDRESS & receiverBalance+amount: NAT | accountOf:=accountOf<+{dst|->receiverBalance+amount})) [] not(senderBalance>=amount & receiverBalance+amount<=MAXINT) ==> skip)));
  Expanded_List_Substitution(Implementation(B_weth_i),withdraw)==(msg_sender: USERS & amount: NAT1 | @(senderAccount,senderBalance).((msg_sender: ADDRESS | senderAccount:=accountOf(msg_sender));(msg_sender: ADDRESS | senderBalance:=balanceOf(msg_sender));(senderAccount>=amount & senderBalance+amount<=MAXINT ==> ((THIS: ADDRESS & msg_sender: ADDRESS & msg_sender/=THIS & amount: NAT & balanceOf(msg_sender)+amount: NAT & balanceOf(THIS)-amount: NAT | balanceOf:=balanceOf<+{THIS|->balanceOf(THIS)-amount,msg_sender|->balanceOf(msg_sender)+amount});(msg_sender: ADDRESS & senderAccount-amount: NAT | accountOf:=accountOf<+{msg_sender|->senderAccount-amount})) [] not(senderAccount>=amount & senderBalance+amount<=MAXINT) ==> skip)));
  ...
  Expanded_List_Substitution(Implementation(B_weth_i),check_account_overflow)==(btrue | @(ii,value).(ii:=0;value:=TRUE;WHILE ii<index & value = TRUE DO @depositorBalance.((depositors_i(ii): ADDRESS | depositorBalance:=accountOf(depositors_i(ii)));value:=bool(depositorBalance+1<=MAXINT);ii:=ii+1) INVARIANT ii<=index & ii>=0 & value = bool(!xx.(xx: ran(0..ii-1<|depositors_i) => accountOf(xx)+1: NAT)) VARIANT index-ii END;ret:=value));
  List_Substitution(Implementation(B_weth_i),check_account_overflow)==(VAR ii,value IN ii:=0;value:=TRUE;WHILE ii<index & value = TRUE DO VAR depositorBalance IN depositorBalance <-- get_accountOf(depositors_i(ii));value:=bool(depositorBalance+1<=MAXINT);ii:=ii+1 END INVARIANT ii<=index & ii>=0 & value = bool(!xx.(xx: ran(0..ii-1<|depositors_i) => accountOf(xx)+1: NAT)) VARIANT index-ii END;ret:=value END);
  ...
  List_Substitution(Implementation(B_weth_i),withdraw)==(VAR senderAccount,senderBalance IN senderAccount <-- get_accountOf(msg_sender);senderBalance <-- get_balanceOf(msg_sender);IF senderAccount>=amount & senderBalance+amount<=MAXINT THEN transfer(THIS,msg_sender,amount);set_accountOf(msg_sender,senderAccount-amount) END END);
  List_Substitution(Implementation(B_weth_i),transferTo)==(VAR senderBalance,receiverBalance IN senderBalance <-- get_accountOf(msg_sender);receiverBalance <-- get_accountOf(dst);IF senderBalance>=amount & receiverBalance+amount<=MAXINT THEN set_accountOf(msg_sender,senderBalance-amount);set_accountOf(dst,receiverBalance+amount) END END);
  ...
  List_Substitution(Implementation(B_weth_i),rewardTopDepositors)==(VAR thisBalance,managerBalance,safe IN safe <-- check_account_overflow;thisBalance <-- get_balanceOf(THIS);managerBalance <-- get_balanceOf(manager_i);IF msg_value = threshold_i & index = threshold_i & donated_i = FALSE & thisBalance+msg_value<=MAXINT & managerBalance-msg_value>=0 & safe = TRUE THEN transfer(msg_sender,THIS,msg_value);donated_i:=TRUE;VAR ii IN ii:=0;WHILE ii<index DO VAR depositorBalance IN depositorBalance <-- get_accountOf(depositors_i(ii));set_accountOf(depositors_i(ii),depositorBalance+1);ii:=ii+1 END INVARIANT ii = 100 or ii: dom(depositors_i) & accountOf = accountOf$0<+%xx.(xx: depositors_i[0..ii-1] | accountOf$0(xx)+1) VARIANT 100-ii END END END END)
END
&
...
THEORY InheritedEnvX IS
  ...
  VisibleVariables(Implementation(B_weth_i))==(Type(manager_i) == Mvv(etype(ADDRESS,?,?));Type(depositors_i) == Mvv(SetOf(btype(INTEGER,0,99)*etype(ADDRESS,?,?)));Type(index) == Mvv(btype(INTEGER,?,?));Type(donated_i) == Mvv(btype(BOOL,?,?)));
...
THEORY ListOfIdsX IS
  List_Of_Ids(Implementation(B_weth_i)) == (threshold_i | ? | ? | depositedOver_100,allowanceOf,accountOf,balanceOf | check_account_overflow,deposit,withdraw,transferTo,approve,transferFrom,rewardTopDepositors,refinement_of_check_account_overflow | ? | seen(Machine(Solidity_Types)),imported(Machine(Platform)),imported(Machine(account)),imported(Machine(allowance)),imported(Machine(depositedOver100)) | ? | B_weth_i);
...
THEORY VisibleVariablesEnvX IS
  VisibleVariables(Implementation(B_weth_i)) == (Type(donated_i) == Mvv(btype(BOOL,?,?));Type(index) == Mvv(btype(INTEGER,?,?));Type(depositors_i) == Mvv(SetOf(btype(INTEGER,0,99)*etype(ADDRESS,?,?)));Type(manager_i) == Mvv(etype(ADDRESS,?,?)))
END
&
THEORY VariablesLocEnvX IS
  Variables_Loc(Implementation(B_weth_i),check_account_overflow, 2) == (Type(depositorBalance) == Lvl(btype(INTEGER,?,?)));
  Variables_Loc(Implementation(B_weth_i),check_account_overflow, 1) == (Type(ii) == Lvl(btype(INTEGER,?,?));Type(value) == Lvl(btype(BOOL,?,?)));
  ...
  Variables_Loc(Implementation(B_weth_i),withdraw, 1) == (Type(senderAccount) == Lvl(btype(INTEGER,?,?));Type(senderBalance) == Lvl(btype(INTEGER,?,?)));
  ...
  Variables_Loc(Implementation(B_weth_i),rewardTopDepositors, 2) == (Type(ii) == Lvl(btype(INTEGER,?,?)));
  Variables_Loc(Implementation(B_weth_i),rewardTopDepositors, 1) == (Type(thisBalance) == Lvl(btype(INTEGER,?,?));Type(managerBalance) == Lvl(btype(INTEGER,?,?));Type(safe) == Lvl(btype(BOOL,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == KO;
...
THEORY ListLocalOperationsX IS
  List_Local_Operations(Implementation(B_weth_i))==(check_account_overflow)
END
&
THEORY ListLocalInputX IS
  List_Local_Input(Implementation(B_weth_i),check_account_overflow)==(?)
END
&
THEORY ListLocalOutputX IS
  List_Local_Output(Implementation(B_weth_i),check_account_overflow)==(ret)
END
&
THEORY ListLocalHeaderX IS
  List_Local_Header(Implementation(B_weth_i),check_account_overflow)==(ret <-- check_account_overflow)
END
&
THEORY ListLocalPreconditionX IS
  List_Local_Precondition(Implementation(B_weth_i),check_account_overflow)==(btrue)
END
&
THEORY ListLocalSubstitutionX IS
  Expanded_List_Local_Substitution(Implementation(B_weth_i),check_account_overflow)==(btrue | ret:=bool(!xx.(xx: ran(0..index-1<|depositors_i) => accountOf(xx)+1: NAT)));
  List_Local_Substitution(Implementation(B_weth_i),check_account_overflow)==(ret:=bool(!xx.(xx: ran(0..index-1<|depositors_i) => accountOf(xx)+1: NAT)))
END
&
...
THEORY ListLocalOpInvariantX IS
  LocalOp_Expanded_Invariant(Implementation(B_weth_i),check_account_overflow)==(btrue)
END
)
