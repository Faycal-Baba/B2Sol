...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(allowance))==(allowanceOf: USERS --> (USERS --> NAT))
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(allowance))==(@(allowanceOf$0).(allowanceOf$0: USERS --> (USERS --> NAT) ==> allowanceOf:=allowanceOf$0));
  Context_List_Initialisation(Machine(allowance))==(skip);
  List_Initialisation(Machine(allowance))==(allowanceOf:: USERS --> (USERS --> NAT))
END
&
...
THEORY ListInputX IS
  List_Input(Machine(allowance),set_allowanceOf_abstract)==(updates);
...
THEORY ListHeaderX IS
  List_Header(Machine(allowance),set_allowanceOf_abstract)==(set_allowanceOf_abstract(updates));
...
THEORY ListPreconditionX IS
  List_Precondition(Machine(allowance),set_allowanceOf_abstract)==(updates: USERS +-> (USERS +-> NAT));
  List_Precondition(Machine(allowance),get_allowanceOf)==(key1: USERS & key2: USERS);
  List_Precondition(Machine(allowance),set_allowanceOf)==(key1: USERS & key2: USERS & value: NAT)
END
&
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Machine(allowance),set_allowanceOf)==(key1: USERS & key2: USERS & value: NAT | allowanceOf:=allowanceOf<+{key1|->(allowanceOf(key1)<+{key2|->value})});
  Expanded_List_Substitution(Machine(allowance),get_allowanceOf)==(key1: USERS & key2: USERS | ret:=allowanceOf(key1)(key2));
  Expanded_List_Substitution(Machine(allowance),set_allowanceOf_abstract)==(updates: USERS +-> (USERS +-> NAT) | allowanceOf:=allowanceOf<+updates);
  List_Substitution(Machine(allowance),set_allowanceOf_abstract)==(allowanceOf:=allowanceOf<+updates);
...
THEORY VariablesEnvX IS
  Variables(Machine(allowance)) == (Type(allowanceOf) == Mvl(SetOf(etype(ADDRESS,"[USERS","]USERS")*SetOf(etype(ADDRESS,"[USERS","]USERS")*btype(INTEGER,0,MAXINT)))))
END
&
THEORY OperationsEnvX IS
  Operations(Machine(allowance)) == (Type(set_allowanceOf) == Cst(No_type,etype(ADDRESS,?,?)*etype(ADDRESS,?,?)*btype(INTEGER,?,?));Type(get_allowanceOf) == Cst(btype(INTEGER,?,?),etype(ADDRESS,?,?)*etype(ADDRESS,?,?));Type(set_allowanceOf_abstract) == Cst(No_type,SetOf(etype(ADDRESS,?,?)*SetOf(etype(ADDRESS,?,?)*btype(INTEGER,?,?)))));
